...
1. Download `cx-impact-0.3.1.zip` from the [release](https://github.com/shelasmax/cx-impact/releases/tag/v0.3.1).
...
If you prefer a repository checkout, copy `skills/cx-impact/` from the `v0.3.1` tag into the same destination. Copy the **whole directory**, not only `SKILL.md`.
...
```sh
shasum -a 256 cx-impact-0.3.1.zip
```
...
Give a scenario and a current/target mode when known. A description without code is sufficient for a labelled draft. To request a process map, ask for participants, handovers, decisions and recovery. Ask for the map in English or Russian. The agent sets `locale: "en"` or `"ru"` for matching viewer labels and writes scenario content in that language. Omitted locale preserves Russian for older JSON.
