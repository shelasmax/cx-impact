...
1. Download [cx-impact-0.6.1.zip](https://github.com/shelasmax/cx-impact/releases/download/v0.6.1/cx-impact-0.6.1.zip).
...
For a repository checkout, use `skills/cx-impact/` from tag `v0.6.1`. The short `npx` command follows the repository; the release archive pins the complete package to a version. Once downloaded, manual installation and rendering need no package registry.

The release includes `SHA256SUMS`. On macOS, compare the archive's hash with its line in that file:

```sh
shasum -a 256 cx-impact-0.6.1.zip
```
