...
`hero.png` and `hero.ru.png` are English and Russian product overviews composed from real renderer captures. They show the current customer journey, service blueprint and process from the synthetic online-sales scenario, alongside the quantitative funnel. The frames crop excerpts for the overview; chart contents are not retouched, redrawn or generated as mockups. Full maps and funnel exports appear in the release gallery. The adjacent README caption states that these are real excerpts.

Regenerate them with `scripts/capture-product-overview.cjs`, using the existing Playwright module and Chrome. The script renders canonical synthetic inputs, exports the selected SVG views, captures them, and takes a browser screenshot of the final bilingual composition. It adds only the heading, labels, frames and background around the captures. `hero.manifest.json` records the version, input/rendered HTML hashes, capture hashes and final image hashes. The release builder verifies the image hashes and checks their source/runtime provenance before including them as `product-en.png` and `product-ru.png` in the offline showcase.

The earlier AI-assisted promotional artwork remains available in previous Git tags. The logo itself remains the original AI-assisted asset described below. Generated map SVGs embed the unchanged logo and the MIT notice.
