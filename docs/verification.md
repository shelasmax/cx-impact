...
Sixteen new English/Russian release images are actual Chrome captures: ten complete SVG exports and six interface screenshots, across all four designs. The gallery states which capture method was used; full SVG captures include content outside the viewer viewport. `scripts/capture-release-screenshots.cjs` uses the existing Playwright/Chrome installation and records source, rendered-HTML and image hashes in the public screenshot manifest. It does not generate promotional images or alter the viewer CSS.

The browser run verifies both actual split target IDs, their claim/source bodies, highlights and separate keyboard activation; distinct old/new source text sharing a source ID; and active single-successor playback reset after design, explicit color or effective System color changes. Each reset closes the player, clears selection, cancels delayed advancement and reopens paused at the root. Complete SVG reopen/export checks and quantitative edge cases also pass.
...
| Reproducibility | Explicit 29-member archive (including VERSION), member/byte/integrity checks, extracted check-only/render of both bicycle demos and all ten packaged fixtures, and exact standalone/copy-away rebuilds. The published showcase ZIP includes the skill package, 16 screenshot files, both language guides and four full HTML/JSON/source bundles; extracted local links, exact bytes and four rebuilds are checked. Screenshot hashes must match the current source and rendered HTML. |
