...
function text(parent,value,x,y,width=1000,size=16,fill=colors.text,weight=500){const lines=Core.wrap(String(value),width,size),n=element('text',parent,{x,y,fill,'font-size':size,'font-family':font,'font-weight':weight});lines.forEach((line,i)=>{const t=element('tspan',n,{x,dy:i?size*1.4:0});t.textContent=line;});return Math.max(1,lines.length)*size*1.4;}
function rect(parent,x,y,width,height,fill,stroke='none'){return element('rect',parent,{x,y,width,height,fill,stroke,rx:designChoice==='classic'?6:0});}
function activate(n,callback,label){n.setAttribute('tabindex','0');n.setAttribute('role','button');n.setAttribute('aria-label',label);n.style.cursor='pointer';const call=()=>{focusReturn=n;callback();};n.addEventListener('click',call);n.addEventListener('keydown',e=>{if(['Enter',' '].includes(e.key)){e.preventDefault();call();}});}
function closeDetails(){document.querySelector('.shell').inert=false;$('drawer').hidden=true;$('drawer-backdrop').hidden=true;if(focusReturn?.isConnected)focusReturn.focus();}
function details(item,body,flowNodeId=null){pause();$('detail-stage').textContent=labels[view];$('detail-title').textContent=item.title||item.label||data.title;$('detail-status').textContent=bi('Авторский агрегат','Authored aggregate');$('detail-text').textContent=body;$('detail-related').replaceChildren();$('detail-sources').replaceChildren();for(const id of item.sourceIds||data.sources.map(s=>s.id)){const s=data.sources.find(s=>s.id===id);paragraph($('detail-sources'),`${s.label} · ${s.kind}`,'h3');paragraph($('detail-sources'),s.text);}if(view==='flows'&&walk&&analysis.graph?.nodes.some(node=>node.id===flowNodeId)){const next=document.createElement('button');next.className='tool';next.textContent=bi('Обзор от этого узла','Explore from this node');next.onclick=()=>{walk=flowNodeId;closeDetails();updateWalk();};$('detail-related').append(next);}document.querySelector('.shell').inert=true;$('drawer').hidden=false;$('drawer-backdrop').hidden=false;$('close').focus();}
...
function step(){if(!walk)return;const edges=analysis.graph.edges.filter(e=>e.from===walk&&e.value>0);if(edges.length===1){walk=edges[0].to;updateWalk();return true;}pause();updateWalk();return false;}
function play(){if(timer){pause();return;}if(reduced.matches){step();return;}function tick(){if(!step())return;timer=setTimeout(tick,1200);$('path-play').setAttribute('aria-pressed','true');}tick();}
function updateWalk(){document.querySelectorAll('[data-walk]').forEach(n=>{n.removeAttribute('data-walk');n.style.removeProperty('outline');});if(!walk)return;const n=analysis.graph.nodes.find(n=>n.id===walk);$('path-position').textContent=`${n.title} · ${number(n.value)}`;$('path-caption').textContent=bi('Обзор заданного графа; не движение людей и не скорость трафика. На развилке выберите переход.','Explore the supplied graph; this is not people movement or traffic speed. Select a transition at branches.');$('path-motion').textContent=reduced.matches?bi('Сниженное движение: только статический выбор.','Reduced motion: static selection only.'):bi('Конечный обзор; выключен при открытии.','Finite traversal; off on load.');const selected=document.querySelector(`[data-funnel-node="${walk}"]`);if(selected){selected.dataset.walk='true';selected.style.outline=`3px solid ${colors.text}`;}const choices=$('path-choices');choices.replaceChildren();const edges=analysis.graph.edges.filter(e=>e.from===walk);choices.hidden=!edges.length;for(const e of edges){const b=document.createElement('button');b.className='tool';b.textContent=`${e.label} · ${number(e.value)}`;b.disabled=e.value===0;b.onclick=()=>{pause();walk=e.to;updateWalk();};choices.append(b);}if(analysis.graph.roots.length>1){for(const id of analysis.graph.roots){const b=document.createElement('button');b.className='tool';b.textContent=bi('Корень: ','Root: ')+analysis.graph.nodes.find(n=>n.id===id).title;b.onclick=()=>{pause();walk=id;updateWalk();};choices.append(b);}choices.hidden=false;}}
function download(bytes,type,name){const blob=new Blob([bytes],{type}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),30000);}
...
const csv=document.createElement('button');csv.className='tool';csv.id='funnel-csv';csv.textContent='↓ CSV';csv.onclick=()=>download(Funnel.funnelCSV(data),'text/csv;charset=utf-8','cx-impact-funnel.csv');$('source').after(csv);
$('questions').textContent=bi('Источники и определения','Sources and definitions');$('questions').onclick=()=>{focusReturn=$('questions');details(data,`${scopeText()}\n${data.scope.identityRule}\n${data.scope.orderRule}\n${repeatText()}\n${limits}`);};$('natural').onclick=()=>{fitMode=false;zoom=1;size();};$('fit').onclick=()=>{fitMode=true;size();};$('plus').onclick=()=>{fitMode=false;zoom=Math.min(2,zoom+.1);size();};$('minus').onclick=()=>{fitMode=false;zoom=Math.max(.2,zoom-.1);size();};window.addEventListener('resize',()=>{if(fitMode)size();});
$('path-toggle').onclick=()=>{const open=$('journey-player').hidden;$('journey-player').hidden=!open;$('path-toggle').setAttribute('aria-expanded',String(open));if(open)resetWalk();else{pause();walk=null;updateWalk();}};$('path-close').onclick=()=>{pause();walk=null;$('journey-player').hidden=true;$('path-toggle').setAttribute('aria-expanded','false');updateWalk();};$('path-prev').hidden=true;$('path-next').onclick=()=>{pause();step();};$('path-reset').onclick=resetWalk;$('path-play').onclick=play;
const repeat=document.createElement('section');repeat.id='funnel-repeat';repeat.className='funnel-context';paragraph(repeat,repeatText());$('map-panel').after(repeat);const context=document.createElement('section');context.id='funnel-definitions';context.className='funnel-context';for(const s of [data.scope.identityRule,data.scope.orderRule,limits,...(data.transitions?[data.transitions.scope.denominator,data.transitions.scope.description]:[])])paragraph(context,s);repeat.after(context);
const style=document.createElement('style');style.textContent='.funnel-context{margin:20px 0;padding:16px 24px;background:var(--surface);border:1px solid var(--border);border-radius:10px}.funnel-context p{max-width:1100px;line-height:1.6}.funnel-table{padding:20px;overflow:auto}.funnel-table table{border-collapse:collapse;min-width:1000px;width:100%;font-variant-numeric:tabular-nums}.funnel-table th,.funnel-table td{text-align:left;padding:14px 12px;border-bottom:1px solid var(--border);vertical-align:top}.funnel-table caption{text-align:left;padding:0 0 20px}.funnel-table button{white-space:normal;text-align:left}#viewport svg{display:block;flex:none}#viewport [role=button]:focus-visible{outline:3px solid var(--focus);outline-offset:4px}#detail-text{white-space:pre-wrap}.controls{flex-wrap:wrap}.tools{flex-wrap:wrap}';document.head.append(style);applyAppearance();
