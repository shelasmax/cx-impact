...
A revision updates a supplied JSON from new material; it does not imply an AS-IS / TO-BE comparison. Preserve mode, scenario boundary, unchanged claims and their stable IDs. Use a new source ID for a new document revision rather than rewriting the text behind an unchanged claim. An updated requirement is still a requirement until actual behavior is supported. Do not infer removal from silence in the new material.

For each changed statement, check affected stage cells, shared barriers, nodes and transitions together. Retain references and full branch conditions. If documents disagree and neither establishes authority, represent the disputed value as unknown, link both sources in its detail, and keep an affected-stage question. If a supplied decision explicitly replaces an old rule, the active claim can use the replacement with its own source; record what changed. A newer date alone does not resolve a conflict.

For ordinary maps, write the revised JSON under a new basename and render a new bundle. For a map with customized snapshot assets:

1. Create a new, empty revision directory. Copy only the old adjacent JSON and the complete `.source/` directory there, keeping their original basenames. The copied `rebuild.mjs` uses relative paths: keeping both items together prevents it from targeting the original directory. Do not copy old browser/visual acceptance reports.
2. Edit the copied JSON, then run the copied `.source/rebuild.mjs`. This uses the copied renderer, core, viewer and template; the installed package is not substituted. Check the rebuild script first if it was customized to use absolute paths.
3. The existing rebuild command regenerates HTML/checks, but does not refresh the snapshot manifest. After successful rendering, refresh only the copied manifest's `sourceSha256` and `htmlSha256` from the revised adjacent files. Preserve its renderer version and asset hashes; if an asset was deliberately edited, refresh its corresponding hash too and record that edit. Use SHA-256 of the actual bytes.
4. Repeat the rebuild and compare HTML bytes before claiming reproducibility. Verify the original bundle is unchanged. Any browser verification applies to the new artifact, not to the previous version.

Write `<basename>.changes.md` next to the revised map, in the user's language:

- **Baseline:** previous JSON/bundle location and source date or explicit unknown date.
- **New material:** source IDs, dates, and what each can establish.
- **Changes:** changed, added and removed entity IDs, before → after meaning, reasons and source IDs. For edges without IDs, use endpoints and full conditions. Say explicitly when there are no removals.
- **Conflicts and unknowns:** affected IDs, competing sources, unresolved question and what decision would resolve it.
- **Verification:** checks run and their results; separate structural, geometry, visual, interaction, export and reproducibility evidence.

This note is authored by the agent, not generated by the renderer. It adds no map fields. The [worked update](quickstart.md) illustrates the workflow with synthetic material.

## Design, themes and path walkthrough

The bundled viewer preserves **Classic**, the original green atlas design, as the default: Avenir Next/Trebuchet typography, rounded tinted cards and green stage headers. **Graphite Field Notes** is an optional design: neutral paper/graphite surfaces, editorial titles, system sans-serif map text, a restrained terracotta selection accent, and separate evidence colors. **Workshop** adds warm paper/charcoal, sage and periwinkle group materials, italic display type and deterministic inset outlines on grouping containers. **Signal** uses cool technical surfaces, compact monospaced step identifiers, square cards and explicit route ports. All four designs have light and dark palettes using semantic tokens, explicit SVG fills/strokes, complete status labels and the existing line patterns. No webfont or rendering service is fetched. Body text remains 16px at native 100%; fit is an overview.

Two independent groups control appearance: Classic / Graphite / Workshop / Signal and Light / Dark / System. The initial defaults are Classic and System. Choices are saved independently in browser-local storage when available. Denied or invalid storage does not prevent switching. Appearance settings are viewer preferences, not new map fields. SVG exports retain selected colors, typography and permanent group outlines. The shared `assets/design-core.js` applies these attributes to ordinary maps, both paired regions, stage bars and Sankey labels; table material is controlled by the same tokens. Decorative shapes never change quantitative widths or flow geometry. Signal is fully recognizable with motion off; the existing finite walkthrough remains optional and reduced-motion stays static.
...
Check JSON before rendering without creating or replacing any files:

```sh
node scripts/render-map.mjs --check path/to/map.json
```

Run from the installed skill directory, or use the actual renderer path. This mode accepts one input and no output path, `--template` or `--no-bundle`. For structurally valid input, stdout contains the same JSON report as `inspectMap`: structure warnings, per-scenario geometry diagnostics and `not_checked` for browser, export and reproducibility checks. Exit code is 0 when structure/geometry pass (warnings are allowed), and 1 for geometry failure. Invalid JSON, broken references, unreadable input or usage errors produce a diagnostic on stderr and exit 1; no success report is emitted. Normal rendering syntax is unchanged.
...
Text labels and symbols distinguish `Δ Changed`, `= Unchanged`, `− Only AS IS · unmatched`, and `+ Only TO BE · unmatched`. “Only” means that side of the authored alignment; it does **not** prove removal or addition in the real service. Comparison includes claim text, status, referenced source bodies and outgoing process conditions. Identity and layout coordinates are excluded. Node stage/lane assignments are compared separately through explicit stage/lane correspondence; reassignment to a different aligned stage or lane is a content change, while renaming an explicitly corresponding ID is not. Source updates can therefore mark otherwise identical claims changed. Changes describe scenario content, not revisions, measured improvement or verified future outcomes.
